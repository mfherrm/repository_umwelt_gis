...
function drawMapSol(data) {
    ...
    i == 0 ? (projectionM = d3.geoMercator().fitSize([width, height]).translate([0, 0]).scale(1)) :
        (d3.geoAzimuthalEqualArea().scale(.4).translate([0.005, -0.02]).rotate([-10, -52]))  //1.left/right (lon) 2.up/down (lat)    
    ...
    var bboxM = pathM.bounds(data),
        s = .92 / Math.max((bboxM[1][0] - bboxM[0][0]) / width, (bboxM[1][1] - bboxM[0][1]) / height),
        t = [(width - s * (bboxM[1][0] + bboxM[0][0])) / 2, (height - s * (bboxM[1][1] + bboxM[0][1])) / 2];
